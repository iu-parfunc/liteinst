...
void foo() {
    ...
    if (straddler) {
      ...
      //  8 Bytes   8 Bytes          
      // |******pp|ppp*****|  -- ps potential probe site
      // \         stradle_part_2_start 
      //  stradle_part_1_start 

      straddle_part_1_start = straddle_point - 1;

      if ( ((uint64_t)straddle_point) % 8 != 0) printf ("****************\n WOWOWOWOW \n **************\n");
      if ( ((uint64_t)straddle_part_1_start) % 8 != 0) printf("****************\n HEYHEYHEY \n **************\n");
      ...
      // Actually it can.
      //if (cutoff_point < 1 || cutoff_point > 4) printf("this cant be right!!!!!!!!!!!!!!!!!\n");
      ...
      uint64_t temp0 = deactiveSequence & get_msb_mask(cutoff_point);
      shift_size = (8 * (8-cutoff_point)); // redundant 
      // temp0 = temp0;
      uint64_t temp1 = activation_sequence_1 & (get_msb_mask(8-cutoff_point));
      deactivation_sequence_1 = (temp0 >> shift_size) | temp1;

      temp0 = deactiveSequence & (get_lsb_mask(8-cutoff_point));
      shift_size = (8 * cutoff_point);
      //temp0 = (temp0);
      temp1 = activation_sequence_2 & get_lsb_mask(cutoff_point); 
      deactivation_sequence_2 = temp0 | (temp1 >> shift_size) ;
      /* uint64_t temp0 = deactiveSequence & get_lmb_mask(cutoff_point); */
      /* shift_size = (8 * (8-cutoff_point)); // redundant  */
