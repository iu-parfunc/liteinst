...
void minimalBackoffPrologFunction(ProbeArg func_id) {
  ...
  if (!allocated) {
    ...
    ((BackoffProfiler*)PROFILER)->
      registerThreadStatistics(current_thread_stats);
    all_thread_stats = 
      ((BackoffProfiler*) PROFILER)->getThreadStatistics();
  }

  //BackoffProfilerStat* global_func_stats = &((BackoffProfilerStat*) g_ubiprof_stats)[func_id];
  TLSBackoffProfilerStat* local_func_stats = 
    &current_thread_func_stats_table[func_id];
...
void minimalBackoffEpilogFunction(ProbeArg func_id) {
  
  ticks epilog_start = getticks();

  BackoffProfilerStat* global_func_stats = 
    &((BackoffProfilerStat*) g_ubiprof_stats)[func_id];
  TLSBackoffProfilerStat* local_func_stats = 
    &current_thread_func_stats_table[func_id];

  int thread_count = ((BackoffProfiler*)PROFILER)->getThreadCount(); 
...
void MinimalBackoffProfiler::initialize() {
  ...
  // C++ value initialization.. Similar to calloc
  tls_stats = new TLStatistics*[64](); 
  ...
  fprintf(stderr, "[Minimal Backoff Profiler] **** Parameters :" 
      " Sample size => %lu\n", mfb_sample_size); 
...
void MinimalBackoffProfiler::dumpStatistics() {
  ...
  for(int i = 0; i < func_count; i++) {
    ...
    for(int j=0; j < thread_counter; j++) {
      TLSBackoffProfilerStat* tls_func_stat = 
        (TLSBackoffProfilerStat*) tls_stats[j]->func_stats;
