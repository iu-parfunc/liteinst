
#include <stdlib.h>
#include <inttypes.h> 

#include "globals.hpp"

// Declaring the profiler static member
Profiler* Profiler::profiler_ = NULL;

// Declaring the probe provider static member
ProbeProvider* Profiler::provider_= NULL;

void* g_ubiprof_stats = 0;
volatile uint64_t g_deactivation_count = 0;

volatile bool g_ubiprof_initialized = false;

/* Globals for sampling and adaptive profilers */
uint64_t sp_initial_sample_size;
uint64_t sp_sample_size;

/* Accessed by multiple threads */ 
volatile double sp_epoch_period;
double sp_target_overhead;

pthread_t g_monitor_thread;
uint64_t g_probe_overheads = 0;
uint64_t g_probe_count = 0;
/*  */
uint64_t g_total_overhead = 0; // Overhead incurred due to profiling
volatile uint64_t g_total_process_time = 0; // Total process time until last epoch sample
uint64_t g_last_epoch_random = 0; // Random added to last epoch period
uint64_t g_TicksPerNanoSec = 0; // Calibrated ticks per nano second
uint64_t g_call_overhead = 0; // Call overhead calibrated value
uint16_t g_strategy = SLOW_RAMP_UP; // Overhead control strategy to use
uint64_t g_thread_lock = 0;
// Volatile is needed because this is accessed by multiple threads:
volatile uint64_t g_shutting_down_flag = 0;

// Leaf node tracking
uint64_t prolog_leaf_counter = 0;
uint64_t epilog_leaf_counter = 0;

