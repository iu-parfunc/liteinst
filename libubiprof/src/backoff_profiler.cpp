...
void backoffPrologFunction(ProbeArg func_id) {
  ...
  if (!allocated) {
    ...
    ((BackoffProfiler*)PROFILER)->registerThreadStatistics(current_thread_stats);
    all_thread_stats = ((BackoffProfiler*) PROFILER)->getThreadStatistics();
  }
...
BackoffProfiler::BackoffProfiler():Profiler(backoffPrologFunction, 
    backoffEpilogFunction) {
...
BackoffProfiler::~BackoffProfiler() {
  ...
  TLStatistics** tls_stat = ((BackoffProfiler*)PROFILER)->getThreadStatistics();
  int thread_count = ((BackoffProfiler*)PROFILER)->getThreadCount();
