...
void* gettid(void* ann) {
    pthread_t ptid = pthread_self();
    uint64_t threadId = 0;
    memcpy(&threadId, &ptid, std::min(sizeof(threadId), sizeof(ptid)));
    printf("Current thread is %d", threadId);
}

extern const char *__progname;

void* activateProbe(void* ann)
{
	// gettid();
  ...
  zca_row_11_t *row = retrieve_data(__progname);
...
void* deactivateProbe(void*) {

}

void __attribute__ ((constructor)) premain()
{
    printf("Program name with __FILE__ is : %s \n", __progname);

    const char *message = "Profiler Thread";
    pthread_t thread;
    int iret;

    activateProbe(NULL);

    iret = pthread_create( &thread, NULL, gettid, (void*) message);
    pthread_join( thread, NULL);
}

int main(int argc, char *argv[])
{
  printf("Program name with argv[0] is : %s \n", argv[0]);
  // gettid();
  // activateProbe(argv[0]);
  //------------------------------------------------------------
  int x = 5;
